...
<div class="thong-ke-thu-chi">
	...
	<div class="row" style="padding-left:10px;">
		...
		<?php  echo $form->field($model, 'loai',  ['options'=>['class'=>'col-xs-4 col-sm-3']])
    ->dropDownList(
        ArrayHelper::map(LoaiThuChi::find()->orderBy(['loai_thu_chi'=>SORT_ASC])->all(), 'id', 'loai_thu_chi'),
        ['prompt' => '— Chọn loại thu/chi —']
    )->label('Loại thu/chi') ?>

    <?php   echo $form->field($model, 'nguoi_thu_chi_id',  ['options'=>['class'=>'col-xs-4 col-sm-3']])
    ->dropDownList(
        ArrayHelper::map(NguoiThuChi::find()->orderBy(['id'=>SORT_ASC])->all(), 'id', 'ho_ten'),
